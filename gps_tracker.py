...
class GPSTracker:
    ...
    def _get_location_cluster_id(self, location: GPSLocation) -> str:
        ...
        for session in self.location_sessions:
            ...
            if distance <= self.config.gps_tracker.location_threshold:
        ...
        # Make unique
        existing_ids = [session.session_id for session in self.location_sessions]
        counter = 1
    ...
    def _update_current_session(self, location: GPSLocation, location_id: str) -> None:
        ...
        for session in self.location_sessions:
            if session.session_id == location_id:
                ...
                if now - session.end_time <= self.config.gps_tracker.session_timeout:
