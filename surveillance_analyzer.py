...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('logs/analysis/surveillance_analysis.log'),
...
class SurveillanceAnalyzer:
    ...
    def analyze_kismet_data(self, kismet_db_path: str = None, gps_data: list = None) -> dict:
        """Perform complete surveillance analysis on Kismet data"""
        
        print("🔍 Starting Surveillance Analysis...")
        print("=" * 50)
        
        # Find all Kismet databases from past X hours
        if not kismet_db_path:
            ...
            # Filter to databases modified in the past X hours
            current_time = time.time()
            hours_x_ago = current_time - (self.config.analysis_window_hours * 3600)
            
            recent_db_files = [db for db in all_db_files if os.path.getmtime(db) >= hours_x_ago]
        ...
        print("🚨 Analyzing for surveillance patterns...")
        ...
        print(f"📝 Generating surveillance reports:")
        print(f"   📄 Markdown: {report_file}")
        print(f"   🌐 HTML: {html_file}")
        self.detector.generate_surveillance_report(report_file)
...
def main():
    ...
    parser.add_argument('--kismet-db', type=str, help='Path to specific Kismet database file')
    parser.add_argument('--gps-file', type=str, help='JSON file with GPS coordinates')
    parser.add_argument('--stalking-only', help='Focus analysis on stalking detection')
    parser.add_argument('--output-json', type=str, help='Export results to JSON file')
    parser.add_argument('--min-threat', type=float, default=0.5, help='Minimum threat score for reporting (default: 0.5)')
    
    args = parser.parse_args()

    try:
        analyzer = SurveillanceAnalyzer(config_path=args.config_path)
        
        # Load GPS data if provided
        gps_data = None
        if args.gps_file:
            with open(args.gps_file, 'r') as file:
        ...
        if args.stalking_only:
            ...
            if stalking_devices:
                print(f"🚨 STALKING ALERT: {len(stalking_devices)} devices with stalking patterns!")
                ...
                print("✅ No stalking patterns detected")
        
        # Export JSON if requested
        if args.output_json:
            analyzer.export_results_json(results, args.output_json)
        
        print("🔒 Analysis complete! Stay safe out there.")
