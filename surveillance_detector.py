...
from src.ConfigHelper import Config
...
class SurveillanceDetector:
    ...
    def analyze_surveillance_patterns(self) -> List[SuspiciousDevice]:
        ...
        for mac, appearances in self.device_history.items():
            if len(appearances) < self.config.detector_thresholds.min_appearances:
                continue
                
            persistence_score, reasons = self._calculate_persistence_score(appearances)
            
            if persistence_score > self.config.detector_thresholds.min_persistence_score:  # Persistence threshold
    ...
    def generate_surveillance_report(self, output_file: str) -> str:
        ...
        report.append("```")
        report.append(f"Minimum Appearances Threshold: {self.config.detector_thresholds.min_appearances}")
        report.append(f"Minimum Time Span: {self.config.detector_thresholds.min_time_span_hours:.1f} hours")
        report.append(f"Minimum Persistence Score: {self.config.detector_thresholds.min_persistence_score}")
        report.append("```")
        report.append("")
...
def load_appearances_from_kismet(db_path: str, detector: SurveillanceDetector, location_id: str = "unknown") -> int:
